import { parseDateTime, parseDateOnly } from './utils.js';
import { setShapeFilter, setCountryFilter } from './filters.js';

// charts.js
// This module renders the analytics dashboard charts. It computes statistics
// from the NUFORC dataset and exposes helpers used by the main analytics
// entrypoint.
...
export function createChartCards() {
...
// previously this file handled bootstrapping and data fetching. In the new
// modular structure those responsibilities live in analytics.js

export function initChartDescriptions() {
  const descriptions = {
...
export async function renderDashboard(data) {
  ...
  for (const chart of CHART_DEFINITIONS) {
    switch (chart.id) {
      case 'chartMap': {
        const { renderLeafletMap } = await import('./map.js');
        renderLeafletMap(metrics.latLonPoints, data);
        break;
      }
      case 'chartShapeTop':
      ...
      case 'chartShapeTrendsDecade':
        ...
        break;
      default:
        console.warn('Unknown chart type for', chart.id);
    }
  }
